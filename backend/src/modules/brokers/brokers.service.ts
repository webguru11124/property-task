...
export class BrokersService {
  ...
  async findNearestByHaversine(lat: number, lng: number, limit = 3) {
    ...
    return entities.map((broker, i) => ({
      ...broker,
      distance: Number((raw[i] as Record<string, unknown>)?.distance_km),
    }));
  }
  ...
  async getRecommendedBrokers(cityId: string) {
    ...
    if (local.length > 0) {
      return {
        brokers: local,
        type: 'local',
        message: `Found ${local.length} broker offices in your city`,
      };
    }
    const city = await this.cityRepository.findOne({ where: { id: cityId } });
    if (!city) throw new Error('City not found');
    const nearest = await this.findNearestByHaversine(
    ...
    return {
      brokers: nearest,
      type: 'nearby',
      message: 'No offices in your city, showing nearest offices',
    };
  }
}
