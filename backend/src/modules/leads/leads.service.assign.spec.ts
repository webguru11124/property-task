...
describe('LeadsService.assignLead', () => {
    function repo<T extends ObjectLiteral>(impl: Partial<Repository<T>>): Repository<T> {
    ...
    it('assigns broker and sends notification when email present', async () => {
        ...
        const leadRepo = repo<Lead>({
            ...
            save: jest.fn().mockImplementation(async (l: any) => ({ ...lead, ...l })),
        })
        const cityRepo = repo<City>({} as any)
        const brokerRepo = repo<BrokerOffice>({ findOne: jest.fn().mockResolvedValue(broker) })
        const brokersService: any = { getRecommendedBrokers: jest.fn() }
        const notificationService: any = { notifyBrokerByEmail: jest.fn().mockResolvedValue({ delivered: true }) }

        const service = new LeadsService(leadRepo, cityRepo, brokerRepo, brokersService, notificationService)
        ...
        expect(brokerRepo.findOne).toHaveBeenCalledWith({ where: { id: 'broker-1' } })
        expect(notificationService.notifyBrokerByEmail).toHaveBeenCalledWith('b@example.com', expect.any(Object))
